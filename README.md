...
```bash
cd /workspace/def-ml/
uv run python convert_data.py --dataset [bigenough/gong-surakav]
```
...
Repeat the below steps for `bigenough` and `gongsurakav` replacing `[dataset]`
below.
...
```bash
cd /workspace/def-ml
scripts/overview.sh [dataset] --truncated
```
...
```bash
cd /workspace/def-ml
uv run python scripts/overview_table.py -en Ephemeral-[dataset]-aug1-infinite Ephemeral-[dataset]-aug1-bottleneck
```

output is printed in `stdout`, latex formatted table is in `tables/table.tex`.

#### Main Result 5: WF Infinite Training Table
...
#### Main Result 6: WF Cross Attack Heatmaps

- Time: 1 human minute, 12 compute hours.

This recreates the Heatmaps in Figures 5 and 7-10.

Here we can leverage the runs from previous steps. Note, you need to have run the steps 4
and 5 in order to access the pretrained models we depend here.

First set (for determinism):
...
```bash
uv run python scripts/cross_attack_defense_heatmap.py -en Ephemeral-[dataset]-[aug1/inftrain] --network [infinite/bottleneck/both] --model [df/rf/df-multi/laserbeak_wo_attention/laserbeak]
```

output found in `figs/*`.

#### Main Result 7: Ephemeral Blocking Defenses vs Laserbeak w/o Attention
...
have generated the ephemeral blocking defenses and run those on undefended
traffic in step 4, e.g., "full evaluation".
