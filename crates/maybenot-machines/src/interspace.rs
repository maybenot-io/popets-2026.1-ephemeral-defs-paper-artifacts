...
pub fn interspace_client<R: RngCore>(rng: &mut R) -> Vec<Machine> {
    ...
    let mut padding = if rng.gen_bool(0.5) {
        State::new(enum_map! {
...
fn interspace_server_manual<R: RngCore>(rng: &mut R) -> Vec<Machine> {
    ...
    if rng.gen_bool(0.5) {
        ...
        let alpha = rng.gen_range(0.01..1000.0);
        let beta = rng.gen_range(0.01..10000.0);
...
fn random_pareto<R: RngCore>(start: f64, max: f64, rng: &mut R) -> Dist {
    Dist {
        dist: DistType::Pareto {
            scale: rng.gen_range(0.0..10.0),
            shape: rng.gen_range(0.0..10.0),
        },
        start,
        max,
    }
...
fn random_log_logistic<R: RngCore>(start: f64, max: f64, rng: &mut R) -> Dist {
    ...
    // param1 is alpha
    let alpha = rng.gen_range(0.01..10.0);
    // param2 is beta
    let beta = rng.gen_range(0.01..10.0);
