...
mod tests {
    ...
    use maybenot_simulator::{network::Network, parse_trace, sim_advanced, SimulatorArgs};
    use std::time::Duration;

    use crate::regulator::{make_regulator_boot_machine, make_regulator_seal_machine};

    use super::get_rate_bins;

    #[test]
    fn test_make_regulator_boot_machine() {
        // we make a machine that receives the signal event and sends an extra
        // padding packet
        let mut states = vec![];
        let start = State::new(enum_map! {
            Event::Signal => vec![Trans(1, 1.0)],
        ...
        let signal = Machine::new(u64::MAX, 0.0, u64::MAX, 0.0, states).unwrap();

        const TRACE: &str = include_str!("BE000.log");
        let m = vec![
            make_regulator_seal_machine(),
            make_regulator_boot_machine(10.0, 10),
            signal,
        ...
        let network = Network::new(delay, Some(20_000));
        let mut args = SimulatorArgs::new(network, 40_000, false);
        args.only_client_events = true;
        let pq = parse_trace(TRACE, network);
        let trace = sim_advanced(&m, &[], &mut pq.clone(), &args);

        // count number of PaddingSent events: 10 from boot, 1 from signal
        let count = trace
            .into_iter()
            .filter(|e| e.event.is_event(Event::PaddingSent))
            .count();
        assert_eq!(count, 10 + 1);
    }

    #[test]
    fn test_get_rate_bins() {
        let (delta, rates) = get_rate_bins(200.0, 0.99, 100);
